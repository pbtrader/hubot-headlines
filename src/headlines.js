...
module.exports = function(robot) {
	...
	robot.respond(/headlines (.*)/i, function(msg){
		...
		// declare these constants in 'makeURL' instead of here
...
// move hours and limit to this function, also API_KEY and base (BASE_URL?) are globals
function makeURL(base,section,hours,limit,API_KEY) {
	const URL = base + section + "/" + hours + ".json?limit=" + limit + "&api-key=" + API_KEY;
	// just return instead of saving to variable and then returning
	return URL;
}
...
function nyTimesAPICall(URL,msg) {
    ...
    // just use one for loop to build the output
...
function helpInstructions(msg) {
  ...
  // e.g. :
  // let helpString = `The headlines blah blah blah
